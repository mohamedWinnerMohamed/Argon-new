...
import { editMovables } from "@/apis/movable-crud";
...
import {
  editMovableSchema,
  EditMovableSchemaType,
} from "@/schemas/edit-movable";
...
export default function MovablesForm({
  ...
  initial: Sender & { movables: Movable[] };
  senderId: string;
}) {
  const {
    control,
    ...
    watch,
    setValue,
  } = useForm<EditMovableSchemaType>({
    resolver: yupResolver(editMovableSchema),
    defaultValues: {
      senderName: initial.name,
      senderPhone: initial.phone,
      movables: initial.movables.map((movable) => ({
        id: movable.id,
        name: movable.name,
        price: movable.price,
  ...
  async function onSubmit(data: EditMovableSchemaType) {
    setIsPending(true);

    const res = await apiErrorHandler(editMovables({ ...data, senderId }));
    ...
    router.push(`/movables-receipt?id=${senderId}`);
  }
