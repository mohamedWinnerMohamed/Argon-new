...
import { CalendarDots as CalendarIcon } from "@phosphor-icons/react";
import {
  endOfHour,
  endOfMinute,
  format,
...
export function DateTimePicker({
  ...
  const dislayFormat = useMemo(() => {
    ...
    return format(
      displayValue,
      `${!hideTime ? "MMM" : "MMMM"} d, yyyy${!hideTime ? (use12HourFormat ? " hh:mm a" : " HH:mm") : ""}`,
      {
        ...(props.locale
          ? {
              locale: props.locale as Pick<
                Locale,
                "localize" | "options" | "formatLong"
              >,
            }
          : {}),
      },
  ...
  return (
    <Popover open={open} onOpenChange={setOpen} modal={modal}>
      ...
      <PopoverContent avoidCollisions className="w-auto p-2">
...
function TimePicker({
  ...
  const ampmOptions = useMemo(() => {
    ...
    return [
      { value: AM_VALUE, label: "ص" },
      { value: PM_VALUE, label: "م" },
    ].map((v) => {
      let disabled = false;
